...
Pipe operator allows us to type in the order we want to process. 
Instead of mutate(select(filter(obj))) it would be 
obj %>% 
filter() %>% 
select() %>% 
mutate()

Shortcut to insert pipe operator is command + shift + m
...
## Export a figure
ggsave uses last created graph in the code. To specify plot to save in ggsave use plot = plot name here
```{r}
